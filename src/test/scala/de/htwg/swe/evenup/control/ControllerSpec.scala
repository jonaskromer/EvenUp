...
import de.htwg.swe.evenup.model.state.{MainMenuState, InGroupState, InEmptyGroupState}
...
class ControllerSpec extends AnyWordSpec with Matchers:
  ...
  "Controller" should {
    ...
    "not add duplicate group" in {
      ...
      observer.lastEvent shouldBe defined
      observer.lastEvent.get match
        case EventResponse.AddGroup(AddGroupResult.GroupExists, _) =>
          succeed
    ...
    "goto empty group" in {
      ...
      observer.lastEvent shouldBe defined
      observer.lastEvent.get match
        case EventResponse.GotoGroup(GotoGroupResult.SuccessEmptyGroup, _) =>
          succeed
    ...
    "fail to goto non-existent group" in {
      ...
      observer.lastEvent shouldBe defined
      observer.lastEvent.get match
        case EventResponse.GotoGroup(GotoGroupResult.GroupNotFound, _) =>
          succeed
    ...
    "not add duplicate user to group" in {
      ...
      observer.lastEvent shouldBe defined
      observer.lastEvent.get match
        case EventResponse.AddUserToGroup(AddUserToGroupResult.UserAlreadyAdded, _, _) =>
          succeed
    ...
    "fail to add expense when paid_by not in group" in {
      ...
      observer.lastEvent shouldBe defined
      observer.lastEvent.get match
        case EventResponse.AddExpenseToGroup(AddExpenseToGroupResult.PaidByNotFound, _) =>
          succeed
    ...
    "fail to calculate debts when no active group" in {
      ...
      observer.lastEvent shouldBe defined
      observer.lastEvent.get match
        case EventResponse.CalculateDebts(CalculateDebtsResult.NoActiveGroup, _) =>
          succeed
