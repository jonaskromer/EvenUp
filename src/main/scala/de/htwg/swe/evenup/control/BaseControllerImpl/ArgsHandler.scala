...
import de.htwg.swe.evenup.model.financial.DebtComponent.BaseDebtImpl.*
...
trait HandlerTemplate:
  ...
  def check(args: Map[String, Any], app: IApp): EventResponse

  def checkOrDelegate(args: Map[String, Any], app: IApp): EventResponse =
...
class ArgsHandler:
  ...
  def checkOrDelegate(args: Map[String, Any], app: IApp): EventResponse = addGroupHandler.checkOrDelegate(
    args,
    app
  )
      ...
      case Some("addExpenseToGroup") =>
        val parsed =
          for
            ...
            date         <- Some(args.get("date").collect { case d: IDate => d }.getOrElse(Date(1, 1, 2000)))
              ...
              case Some(active_group) =>
                ...
                shares match
                  case Some(shares_string) =>
                    ShareParser.parseAndValidate(shares_string, amount, active_group.members) match
                      case Right(validShares) =>
