...
final case class Expense(name: String, amount: Double, date: String, paid_by: Person, shares: Map[Person, Double]):
    ...
    def updateAmount(amount: Double): Expense =
        copy(amount = amount)

    def updateDate(date: String): Expense =
        copy(date = date)

    def updatePaidBy(paid_by: Person): Expense =
        copy(paid_by = paid_by)

    def updateShares(shares: Map[Person, Double]): Expense =
        copy(shares = shares)