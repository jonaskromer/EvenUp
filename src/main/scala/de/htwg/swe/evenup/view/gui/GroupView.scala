...
import scalafx.scene.chart.{LineChart, NumberAxis, XYChart, BarChart, CategoryAxis}
...
class GroupView(controller: IController, group: IGroup, loadingIndicator: ProgressIndicator) {
  ...
  private def createMembersPane(): VBox = {
    val membersList = new ListView[String] {
      items = ObservableBuffer(group.members.map(_.name)*)
      prefHeight = 400
    }
    
    val addMemberBtn = new Button {
      text = "+"
  ...
  private def createExpensesPane(): VBox = {
    val expensesList = new ListView[String] {
      items = ObservableBuffer(group.expenses.map(_.toString)*)
      prefHeight = 400
    }
    
    val addExpenseBtn = new Button {
      text = "+"
      style = "-fx-font-size: 24px; -fx-background-color: #27ae60; -fx-text-fill: white; -fx-background-radius: 25;"
      prefWidth = 50
      prefHeight = 50
  ...
  private def showAddExpenseDialog(): Unit = {
    ...
    val paidByCombo = new ComboBox[String] {
      items = ObservableBuffer(group.members.map(_.name)*)
    ...
    val dateBox = new HBox {
      spacing = 5
      alignment = Pos.Center
      children = Seq(dayField, monthField, yearField)
    }
    
    val sharesArea = new TextArea {
      promptText = "Shares (format: Person:Amount_Person:Amount)\nExample: Alice:25.0_Bob:25.0\nLeave empty for equal split"
      prefRowCount = 4
      wrapText = true
      prefWidth = 400
    }
    ...
    val addBtn = new Button {
      ...
      onAction = _ => {
        try {
          ...
          val shares = if (sharesArea.text.value.trim.isEmpty) None else Some(sharesArea.text.value)
          ...
          case e: Exception =>
            showErrorAlert(s"Error: ${e.getMessage}")
        }
      }
    }
  ...
  private def createDebtsPane(): VBox = {
    ...
    val debtsArea = new TextArea {
      editable = false
      prefRowCount = 8
      wrapText = true
      text = {
        val debts = group.calculateDebt()
        if (debts.nonEmpty) debts.map(_.toString).mkString("\n") else "Click 'Calculate Debts' to see results"
      }
    }
    ...
    val barChart = new BarChart(yAxis, xAxis) {
    ...
    val header = new Label("Debt Calculation") {
      style = "-fx-font-size: 18px; -fx-font-weight: bold;"
    }
