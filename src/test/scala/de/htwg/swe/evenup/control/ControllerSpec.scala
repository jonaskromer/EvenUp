  ...
  "Controller" should {
    ...
    "add a group and notify AddGroup event" in:
      ...
      controller.add(new Observer {
        override def update(event: ObservableEvent): Unit = events.append(event.asInstanceOf[ControllerEvent])
      })

      val group = Group("Trip", List(Person("John")), Nil, Nil)
      controller.addGroup(group)

      events.head shouldBe ControllerEvent.AddGroup(AddGroupResult.Success, group)
    ...
    "goto existing group and notify GotoGroup event" in:
      ...
      events.head shouldBe ControllerEvent.GotoGroup(GotoGroupResult.SuccessEmptyGroup, group)
      controller.app.active_group.get shouldBe group
    ...
    "goto non-existing group and notify GroupNotFound" in:
      ...
      events.head shouldBe ControllerEvent.GotoGroup(GotoGroupResult.GroupNotFound, group)
    ...
    "add user to group and notify Success" in:
      ...
      events.head shouldBe ControllerEvent.AddUserToGroup(AddUserToGroupResult.Success, jane)
    ...
    "not add a user twice and notify UserAlreadyAdded" in:
      ...
      events.head shouldBe ControllerEvent.AddUserToGroup(AddUserToGroupResult.UserAlreadyAdded, john)
    ...
    "add an expense with shares and notify Success" in:
      ...
      events.head match
        case ControllerEvent.AddExpense(AddExpenseResult.Success, expense) =>
          ...
          expense.shares.map(_.amount) should contain theSameElementsAs List(30, 20)
    ...
    "notify observers when going to main menu" in:
      var notified = false
      val observer = new de.htwg.swe.evenup.util.Observer:
        override def update(e: de.htwg.swe.evenup.util.ObservableEvent): Unit =
          notified = e match
    ...
    "notify observers when quitting (without exiting JVM)" in:
      ...
      val controller = new Controller(App(Nil, None, None)) {
        override def quit: Unit =
          notifyObservers(ControllerEvent.Quit) // skip System.exit
      }
    ...
    "notify Success when gotoGroup on non-empty group" in:
      ...
      controller.add(observer)
      controller.gotoGroup(group)
      notifiedEvent shouldBe Some(ControllerEvent.GotoGroup(GotoGroupResult.Success, group))
    ...
    "notify NoActiveGroup when adding user without active group" in:
      ...
      notifiedEvent shouldBe Some(ControllerEvent.AddUserToGroup(AddUserToGroupResult.NoActiveGroup, user))
    ...
    "notify PaidByNotFound when adding expense with invalid payer" in:
      ...
      notifiedEvent match
        case Some(ControllerEvent.AddExpense(AddExpenseResult.PaidByNotFound, expense)) =>
    ...
    "correctly adjust remainder shares when dividing expense among members" in:
      ...
      notifiedEvent match
        case Some(ControllerEvent.AddExpense(AddExpenseResult.Success, expense)) =>
    ...
    "correctly assign remainder to first member when dividing expense" in:
      ...
      notifiedEvent match
        case Some(ControllerEvent.AddExpense(AddExpenseResult.Success, expense)) =>
          val aliceShare = BigDecimal(expense.shares.find(_.person == alice).get.amount).setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble
          val bobShare   = BigDecimal(expense.shares.find(_.person == bob).get.amount).setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble
