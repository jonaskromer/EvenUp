...
object ShareParser:
  ...
  def parseShares(input: String): Either[ParseError, List[Share]] =
    if input.trim.isEmpty then Left(ParseError.InvalidFormat("Empty input"))
    else
  ...
  private def parseSingleShare(input: String): Either[ParseError, Share] =
    input.split(":") match
      ...
      case _ => Left(ParseError.InvalidFormat(s"Invalid share format: $input (expected Person:Amount)"))

  private def validateName(name: String, original: String): Either[ParseError, String] =
    val trimmed = name.trim
    if trimmed.isEmpty then Left(ParseError.EmptyPersonName(original))
    else Right(trimmed)
  ...
  private def parseAmount(amountStr: String, original: String): Either[ParseError, Double] =
    amountStr.trim.toDoubleOption match
      ...
      case None         => Left(ParseError.InvalidAmount(original, s"'$amountStr' is not a valid number"))

  private def validateAmount(person: String, amount: Double): Either[ParseError, Double] =
    if amount < 0 then Left(ParseError.NegativeAmount(person, amount))
    else Right(amount)
  ...
  def validateShareSum(
    ...
    if math.abs(sum - totalAmount) > tolerance then Left(ParseError.ShareSumMismatch(sum, totalAmount))
    else Right(shares)
  ...
  def validatePersonsInGroup(
    ...
    if invalidPersons.nonEmpty then Left(ParseError.PersonNotInGroup(invalidPersons))
    else Right(shares)
  ...
  def validSharePatternEither(input: String): Either[String, String] =
    ...
    if pattern.matches(input) then Right(input)
    else Left(s"Invalid share pattern: $input")
