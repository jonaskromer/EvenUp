...
class Tui(controller: Controller) extends Observer {
  ...
  val addUserToGroupHandler: PartialFunction[ControllerEvent, Unit] =
    case ControllerEvent.AddUserToGroup(AddUserToGroupResult.Success, user) =>
      println(f"Added ${user.name} to ${controller.app.active_group.get.name}.") // IS THIS VALID OR SHOULD I RETURN ALSO A GROUP???
    case ControllerEvent.AddUserToGroup(AddUserToGroupResult.UserAlreadyAdded, user) => 
      println(f"User ${user.name} already added to group ${controller.app.active_group.get.name}!")
    case ControllerEvent.AddUserToGroup(AddUserToGroupResult.NoActiveGroup, user) =>
  ...
  val expenseHandler: PartialFunction[ControllerEvent, Unit] = 
    ...
    case ControllerEvent.AddExpense(AddExpenseResult.ActiveGroupNotFound, expense) => 
    ...
    case ControllerEvent.AddExpense(AddExpenseResult.SharesPersonNotFound, expense) =>
      println(s"Wrong user in shares.")
    case ControllerEvent.AddExpense(AddExpenseResult.PaidByNotFound, expense) =>
      println(s"Please first add ${expense.paid_by} to the group before using in expense.")
