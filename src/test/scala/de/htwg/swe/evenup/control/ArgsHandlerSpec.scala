...
import de.htwg.swe.evenup.model.financial.{Expense, Share, ExpenseBuilder}
  ...
  "ArgsHandler" should {
    ...
    "handle gotoGroup operation" when {
      "group exists and is empty" in {
        ...
        response match
          case EventResponse.GotoGroup(GotoGroupResult.SuccessEmptyGroup, g) =>
      ...
      "group exists and has members" in {
        ...
        response match
          case EventResponse.GotoGroup(GotoGroupResult.Success, g) =>
            g.name shouldBe "TestGroup"
      ...
      "group does not exist" in {
        ...
        response match
          case EventResponse.GotoGroup(GotoGroupResult.GroupNotFound, g) =>
            g.name shouldBe "NonExistent"
    ...
    "handle addUserToGroup operation" when {
      "user is added successfully" in {
        ...
        response match
          case EventResponse.AddUserToGroup(AddUserToGroupResult.Success, user, _) =>
      ...
      "no active group is set" in {
        ...
        response match
          case EventResponse.AddUserToGroup(AddUserToGroupResult.NoActiveGroup, user, _) =>
            user.name shouldBe "Alice"
    ...
    "handle undo operation" when {
      "undo stack is not empty" in {
        ...
        response match
          case EventResponse.Undo(UndoResult.Success, size) =>
            size shouldBe 2
      ...
      "undo stack is empty" in {
        ...
        response match
          case EventResponse.Undo(UndoResult.EmptyStack, size) =>
            size shouldBe 0
    ...
    "handle redo operation" when {
      "redo stack is not empty" in {
        ...
        response match
          case EventResponse.Redo(RedoResult.Success, size) =>
            size shouldBe 1
      ...
      "redo stack is empty" in {
        ...
        response match
          case EventResponse.Redo(RedoResult.EmptyStack, size) =>
            size shouldBe 0
    ...
    "handle addExpenseToGroup operation" when {
      ...
      "expense with remainder distribution" in {
        val argsHandler = new ArgsHandler
        val group = Group("TestGroup", List(Person("Alice"), Person("Bob"), Person("Charlie")), Nil, Nil, NormalDebtStrategy())
        val app = App(List(group), None, Some(group), InGroupState())
    ...
    "handle setDebtStrategy operation" when {
      ...
      "no active group" in {
        ...
        response match
          case EventResponse.SetDebtStrategy(SetDebtStrategyResult.NoActiveGroup, _) =>
            succeed
