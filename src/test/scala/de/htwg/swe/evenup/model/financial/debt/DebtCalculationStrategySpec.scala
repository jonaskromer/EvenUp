...
class DebtCalculationStrategySpec extends AnyWordSpec with Matchers:
  ...
  "A DebtCalculationStrategy" should {
    ...
    val expense2 = Expense("Dinner", 60.0, Date(2,1,2025), bob, List(Share(alice, 20.0), Share(bob, 20.0), Share(charlie, 20.0)))

    val group = Group("Trip", List(alice, bob, charlie), List(expense1, expense2), List(), new DebtCalculationStrategy {
