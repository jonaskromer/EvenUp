...
class BorderDecorator(component: TuiComponent, borderChar: String = "*") extends ComponentDecorator(component):
...
class ColorDecorator(component: TuiComponent, colorCode: String) extends ComponentDecorator(component):

  override def render: String = s"$colorCode${component.render}${ConsoleColors.RESET}"

class PaddingDecorator(component: TuiComponent, padding: Int = 1) extends ComponentDecorator(component):
...
class HeaderFooterDecorator(
  ...
  override def render: String = (header.toSeq ++ Seq(component.render) ++ footer.toSeq).mkString("\n")
